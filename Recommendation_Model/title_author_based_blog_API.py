...
# Load the model
with open('auth_title_blog_recomm.pkl', 'rb') as model_file:
    author_title_similarities = pickle.load(model_file)
...
@app.route('/api/v1/auth/title/recommendations', methods=['POST'])
def get_recommendations():
    data = request.json
    blog_id = data.get('blog_id')
    if blog_id is None:
        return jsonify({"error": "Missing 'blog_id' in request body"}), 400
    recommendations = recommend_blogs(blog_id, author_title_similarities)
